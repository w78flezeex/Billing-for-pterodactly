...
export default function EditPlanPage() {
  ...
  const loadPlan = async () => {
    ...
    try {
      ...
      setServerCount(plan._count.servers)
      setOrderCount(plan._count.orders)
      
      // Specs
      setRam(plan.specs?.ram?.toString() || "2048")
      setCpu(plan.specs?.cpu?.toString() || "100")
      setDisk(plan.specs?.disk?.toString() || "10240")
      setSwap(plan.specs?.swap?.toString() || "0")
      setBackups(plan.specs?.backups?.toString() || "2")
      setDatabases(plan.specs?.databases?.toString() || "0")
      setAllocations(plan.specs?.allocations?.toString() || "1")
      
      // Features
      setFeatures(Array.isArray(plan.features) ? plan.features : [])
      
      // Pterodactyl
      if (plan.pterodactylNestId) {
        setSelectedNestId(plan.pterodactylNestId.toString())
      }
      if (plan.pterodactylEggId) {
        setSelectedEggId(plan.pterodactylEggId.toString())
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const data = await res.json()
        alert(data.error || "Ошибка при сохранении тарифа")
      }
    } catch (error) {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const data = await res.json()
        alert(data.error || "Ошибка при удалении тарифа")
      }
    } catch (error) {
        ...
        >
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-3xl font-bold">Редактирование тарифа</h1>
                <p className="text-muted-foreground">
                  {serverCount} серверов • {orderCount} заказов
                </p>
              </div>
            </div>
            ...
            <AlertDialog>
              <AlertDialogTrigger asChild>
                <Button variant="destructive" disabled={serverCount > 0}>
              ...
              <AlertDialogContent>
                <AlertDialogHeader>
                  ...
                  <AlertDialogDescription>
                    Это действие нельзя отменить. Тариф будет удалён навсегда.
                  </AlertDialogDescription>
                </AlertDialogHeader>
                <AlertDialogFooter>
